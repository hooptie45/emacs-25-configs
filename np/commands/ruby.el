...
;;;###autoload
(defun np/toggle-ruby-block-type ()
...
;;;###autoload
(defun np/toggle-ruby-regex-type ()
      ...
      (if (string-match regex matched)
          (cond ((string= (substring matched 0 1) "/")
                ...
                ((string= (substring matched 0 1) "{")
                 ...
                 (let ((old_point (point)))
                   (insert (concat "/"
                                   (np/regex-replace (substring matched 1 -1)
                                                     "/"
                                                     "\\\\/")
                                   "/"))
                ...
                ((string= (substring matched 0 3) "%r{")
                 ...
                 (let ((old_point (point)))
                   (insert (concat "/"
                                   (np/regex-replace (substring matched 3 -1)
                                                     "/"
                                                     "\\\\/")
                                   "/"))
                   (goto-char (+ old_point 1)))))))))
...
;;;###autoload
(defun np/toggle-ruby-string-and-symbol ()
...
;;;###autoload
(defun np/toggle-ruby-hash-type ()
...
(defmacro np/change-this (name fn)
  `(defun ,name ()
    (interactive)
    (save-excursion
      (let* ((thing (bounds-of-thing-at-point 'sexp))
             (str (buffer-substring-no-properties (car thing) (cdr thing)))
             (newstr (,fn str)))
        (progn
          (delete-region (car thing) (cdr thing))
          (insert newstr))))))

(np/change-this np/camelize-this s-upper-camel-case)
(np/change-this np/underscore-this s-snake-case)
(np/change-this np/dasherize-this s-dashed-words)

(global-set-key (kbd "C-H->") 'np/dasherize-this)
(global-set-key (kbd "C-<") 'np/camelize-this)
(global-set-key (kbd "C->") 'np/underscore-this)
